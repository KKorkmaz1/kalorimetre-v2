...
export default function AddMealModal({ isOpen, onClose }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex max-h-[94vh] w-full max-w-app flex-col rounded-t-3xl bg-white shadow-2xl">
        ...
        <div className="flex-shrink-0 px-5 pt-3 pb-0">
          ...
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-lg font-extrabold text-slate-900">Öğün Ekle</h2>
            ...
            {MEAL_TYPES.map(t => (
              <button key={t} type="button" onClick={() => setMealType(t)}
                className={`rounded-xl py-1.5 text-xs font-bold transition-all ${
                  mealType === t ? 'bg-emerald-500 text-white shadow-sm shadow-emerald-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}>
                {t}
              </button>
            ))}
          ...
          <input
            ...
            className="mb-3 w-full rounded-xl border border-slate-200 px-3 py-2 text-xs font-medium text-slate-700 outline-none transition-all placeholder:text-slate-300 focus:border-emerald-400"
          />
          ...
          {tab === 'manual' && (
            <div className="space-y-3">
              <input type="text" value={mName} onChange={e => { setMName(e.target.value); setError('') }}
                ...
                className="w-full rounded-2xl border-2 border-slate-200 px-4 py-3 text-sm font-medium text-slate-900 outline-none transition-all placeholder:text-slate-300 focus:border-emerald-500" />

              <div className={`rounded-2xl border-2 px-4 py-3 transition-all ${mKcal ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200'}`}>
                <label className="block text-xs font-bold text-emerald-600">Kalori *</label>
                <div className="flex items-baseline gap-2">
                  <input type="number" inputMode="numeric" min="1" value={mKcal}
                    onChange={e => { setMKcal(e.target.value); setError('') }}
                    placeholder="0"
                    className="w-full bg-transparent text-3xl font-extrabold text-slate-900 outline-none placeholder:text-slate-200" />
                  <span className="text-base font-semibold text-slate-400">kcal</span>
                </div>
              </div>

              <div>
                <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">Makrolar (opsiyonel)</p>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { val: mProt, set: setMProt, label: 'Protein', color: 'text-blue-600',  focus: 'focus-within:border-blue-400'  },
                    { val: mCarb, set: setMCarb, label: 'Karb',    color: 'text-amber-600', focus: 'focus-within:border-amber-400' },
                    { val: mFat,  set: setMFat,  label: 'Yağ',     color: 'text-red-500',   focus: 'focus-within:border-red-400'   },
                  ].map(({ val, set, label, color, focus }) => (
                    <div key={label} className={`rounded-2xl border-2 border-slate-200 px-3 py-3 text-center transition-all ${focus}`}>
                      <label className={`block text-[10px] font-bold ${color}`}>{label}</label>
                      <input type="number" inputMode="decimal" min="0"
                        value={val} onChange={e => { set(e.target.value); setError('') }}
                        placeholder="0"
                        className="mt-1 w-full bg-transparent text-center text-xl font-extrabold text-slate-800 outline-none placeholder:text-slate-200" />
            ...
            <div className="space-y-2">
              {recipes.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-200 py-8 text-center">
                  <p className="text-sm font-semibold text-slate-600">Henüz kayıtlı tarifin yok</p>
                ...
                recipes.map(recipe => (
                  <div key={recipe.id} className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white px-4 py-3 shadow-sm">
                    ...
                    <button type="button" onClick={() => addRecipeToBasket(recipe)}
                      className="rounded-xl bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-600 hover:bg-emerald-100 transition-colors">
                      Ekle
                    </button>
        ...
        {/* ── FIXED FOOTER ── */}
        <div className="flex-shrink-0 px-5 pb-8 pt-3 space-y-2 border-t border-slate-100 bg-white">
          {error && (
            <p className={`rounded-xl px-3 py-2 text-xs font-semibold ${
              error.startsWith('✓') ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'
            }`}>
              {error}
            </p>
          )}
          ...
          {basket.length > 0 && (
            <button type="button" onClick={handleSaveRecipe}
              className="flex w-full items-center justify-center gap-1.5 rounded-2xl border-2 border-slate-200 py-3 text-xs font-bold text-slate-600 transition-all hover:border-emerald-300 hover:text-emerald-600 active:scale-95">
