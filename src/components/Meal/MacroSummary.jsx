      ...
      {alternatives.length > 0 && (
        <div className="rounded-2xl border border-slate-200 dark:border-night-border bg-white dark:bg-night-card p-3">
          ...
          <div className="space-y-1.5">
            {alternatives.map(f => (
              <div key={f.id} className="flex items-center justify-between rounded-xl bg-slate-50 dark:bg-night-muted px-3 py-2">
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{f.name}</span>
                <span className="text-[10px] text-slate-400 dark:text-slate-500">{f.calories} kcal · P:{f.protein}g</span>
              </div>
            ))}
          </div>
        </div>
